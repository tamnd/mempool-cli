// Package cli assembles the mempool command tree from the mempool
...
// NewApp assembles the kit application from the mempool domain. The
...
//
// To add a command, declare it in mempool/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := mempool.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(mempool.Domain{}).Register(app)
