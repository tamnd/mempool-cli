...
describes what users get. Pick a format with `-o`, or let mempool choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mempool <command> -o table     # a rounded, color-aware grid for reading
mempool <command> -o markdown  # a GitHub pipe table to paste into docs (alias: md)
mempool <command> -o jsonl     # one JSON object per line, for piping
mempool <command> -o json      # a single JSON array
mempool <command> -o csv       # spreadsheet friendly
mempool <command> -o tsv       # tab-separated
mempool <command> -o url       # just the URL column
mempool <command> -o raw       # the underlying bytes, unformatted
```
...
```bash
mempool <command> --fields id,title,url
```
...
```bash
mempool <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
mempool <command>            # a table, because this is a terminal
mempool <command> | wc -l    # JSONL, because this is a pipe
```
